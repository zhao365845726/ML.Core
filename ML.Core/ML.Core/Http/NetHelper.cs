...
namespace ML.Core
{
    ...
    public class NetHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<AdapterEntity> GetEthernetInfo()
        {
            try
            {
                //参考：https://www.cnblogs.com/yushuo/p/6444072.html?utm_source=itdadao&utm_medium=referral
                //获取本地计算机上网络接口的对象
                NetworkInterface[] adapters = NetworkInterface.GetAllNetworkInterfaces();
                //声明适配器的List
                List<AdapterEntity> lstAdapter = new List<AdapterEntity>();
                foreach (NetworkInterface adapter in adapters)
                {
                    //判断类型是以太网
                    if (adapter.NetworkInterfaceType.ToString().ToUpper() == "ETHERNET")
                    {
                        if (adapter.OperationalStatus.ToString().ToUpper() == "UP")
                        {
                            if (adapter.Description.Contains("Virtual") || adapter.Description.Contains("Hyper"))
                            {
                                continue;
                            }
                            AdapterEntity aeEntity = new AdapterEntity();
                            aeEntity.Description = adapter.Description;
                            aeEntity.Id = adapter.Id;
                            aeEntity.Name = adapter.Name;
                            aeEntity.NetworkInterfaceType = adapter.NetworkInterfaceType.ToString();
                            aeEntity.Speed = adapter.Speed * 0.001 * 0.001 + "M";
                            aeEntity.OperationalStatus = adapter.OperationalStatus.ToString();
                            aeEntity.MacPhysicalAddress = adapter.GetPhysicalAddress().ToString();
